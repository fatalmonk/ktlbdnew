...
export function isLowEndDevice(): boolean {
  ...
  const memory = ('deviceMemory' in navigator ? (navigator as { deviceMemory?: number }).deviceMemory : undefined) || 4;
  
  // Check connection speed (if available)
  const connection = ('connection' in navigator ? (navigator as { connection?: NetworkInformation }).connection : undefined) ||
                     ('mozConnection' in navigator ? (navigator as { mozConnection?: NetworkInformation }).mozConnection : undefined) ||
                     ('webkitConnection' in navigator ? (navigator as { webkitConnection?: NetworkInformation }).webkitConnection : undefined);
