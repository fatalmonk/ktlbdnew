...
export function reportError(error: unknown, context?: Record<string, unknown>): void {
  ...
  console.error(serializePayload('error', err.message, { stack: err.stack, ...context }));
...
export function initGlobalErrorHandlers(): void {
  window.addEventListener('error', (event) => {
    if (event.error) {
      reportError(event.error, { source: 'error', filename: event.filename, lineno: event.lineno });
    } else {
