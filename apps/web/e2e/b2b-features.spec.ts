...
test.describe('B2B Features E2E Tests', () => {
  ...
  test.describe('Certifications', () => {
    ...
    test('should display certification badges', async ({ page }) => {
      ...
      const certificationCards = page.locator('[class*="certification"]').or(page.locator('[class*="badge"]'));
    ...
    test('should filter certifications by category', async ({ page }) => {
      ...
      await page.locator('select').first().selectOption({ label: /Quality/i });
  ...
  test.describe('Chatbot Widget', () => {
    test('should display chatbot button', async ({ page }) => {
      ...
      const chatbotButton = page.locator('button:has([class*="MessageCircle"])').or(
        page.locator('[class*="chatbot"]').first()
      );
  ...
  test.describe('Navigation', () => {
    test('should have updated navigation with new sections', async ({ page }) => {
      await page.goto('/');
      
      // Verify Investor Relations exists
      await expect(page.locator('nav').locator('text=/Investor Relations/i')).toBeVisible();
      
      // Verify Trust & Compliance exists
      await expect(page.locator('nav').locator('text=/Trust.*Compliance/i')).toBeVisible();
    });
  ...
  test.describe('Homepage Enhancements', () => {
    ...
    test('should display sustainability snapshot', async ({ page }) => {
      ...
      await page.locator('text=/Committed to.*Sustainability/i').scrollIntoViewIfNeeded();
      
      // Verify metrics are shown
      await expect(page.locator('text=/Renewable Energy/i')).toBeVisible();
    });

    test('should have CTA buttons linking to RFQ and contact', async ({ page }) => {
      await page.goto('/');
      
      // Scroll to bottom CTA
      await page.locator('text=/Ready to Start Your Next Project/i').scrollIntoViewIfNeeded();
